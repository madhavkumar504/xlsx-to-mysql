...
class Welcome extends CI_Controller {
	...
	function uploadExcel(){
        // echo '<pre>';
        if ( $xlsx = SimpleXLSX::parse( '/opt/lampp/htdocs/ttt.xlsx' ) ) {
            // foreach ( $xlsx->rows() as $r => $row ) {
            //     // foreach ( $row as $c => $cells ) {

				
			// 	// // or $xlsx->toHTML();
				
            //     // //     // echo ($c > 0) ? ', ' : '';
            //     // //     // echo ( $r === 0 ) ? '<b>'.$cells.'</b>' : $cells;
            ...
            //     // echo '<br/>';
            // }
			// $sql = "INSERT INTO `medicine` (`MedicineId`, `medicine_name`, `mrp`, `karmaprice`, `DisableKarmaPrice`, `procurementprice`, `isinjection`, `type`, `Threshold`, `Distributor`, `FastRetrieval`, `KarmaId`, `IsOTC`,`IsMedtronics`,`IsAudioCall`,`IsD2DHomeVisit`,`company`,`version`,`HSNCode`,`ScheduleDrugs`,`Packing`,`GST`)
    ...
    // function uploadExcel(){
    //     $this->load->database();
    //     $inputFile = fopen('/opt/lampp/htdocs/MEDICINELIST.xlsx', "r"); 
    //     // $inputFile = "MEDICINELIST.xlsx";
